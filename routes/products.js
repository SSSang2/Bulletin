...
//전체 게시물의 숫자
    ...
    var queryString = 'select count(*) as cnt from notice'
...
//0보다 크면 limit 함수에 들어갈 첫번째 인자 값 구하기
        ...
        fs.readFile('list.html', 'utf-8', function (error, data) {
            ...
            var queryString = 'select * from notice order by id desc limit ?,?';
...
router.get("/delete/:id", function (req, res) {
    ...
    getConnection().query('delete from notide where id = ?', [req.params.id], function () {
        res.redirect('/main')
    });
...
router.post("/insert", function (req, res) {
    ...
    var body = req.body;
    getConnection().query('insert into notice(title,name,content) values (?,?,?)', [body.title, body.name, body.content], function () {
//응답
        res.redirect('/main');
    })
...
router.get("/edit/:id", function (req, res) {
    ...
    fs.readFile('edit.html', 'utf-8', function (error, data) {
        getConnection().query('select * from notice where id = ?', [req.params.id], function (error, result) {
            res.send(ejs.render(data, {
                data: result[0]
            }))
        })
...
router.post("/edit/:id", function (req, res) {
    ...
    var body = req.body;
    getConnection().query('update notice set title = ?, name = ?, content = ? where id = ?',
        [body.title, body.name, body.content, req.params.id], function () {
            res.redirect('/main')
        })
})
